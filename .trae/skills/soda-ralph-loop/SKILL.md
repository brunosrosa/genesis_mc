...
4. **A Guilhotina FinOps (Escalonamento AST O(1)):**
   ...
   * Acione o `@mcp-jcodemunch-master` para extrair APENAS a Árvore de Sintaxe Abstrata (AST) da função exata que está causando o erro. 
...
* **ESCUDO FINOPS:** É um crime arquitetural enviar um arquivo de 1000 linhas para o Claude Opus só para arrumar um erro de Borrow Checker na linha 42. Use o `jcodemunch` (AST).
...
3. O agente aciona o `jcodemunch`, fatia apenas a função `transmit_ipc` (15 linhas) e despacha o erro exato para a Nuvem (Cloud Brain).
