...
Este documento ancora o repertorio de late-binding da skill `souls-rust-expert`. O foco e Rust bare-metal sob carga local severa, com rejeicao explicita de abstrações confortaveis que escondem custo termodinamico, ambiguidade de ownership ou colapso de throughput. O objetivo nao e escrever Rust "idiomatico de tutorial", mas sim sobreviver a arquiteturas reais com Tokio, IPC binario, inferencia local, sidecars nativos, latencia previsivel e soberania de memoria.
...
Para o ecossistema SOULS, a heuristica e:
