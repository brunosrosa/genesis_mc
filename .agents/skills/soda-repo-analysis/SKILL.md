...
3. **Fase 2: O Enxame Cognitivo (A Tríade Sentido-Estrutura-Realidade):**
   * Dispare paralelamente as três Lentes de análise:
     ...
     * **Lente B (Estrutura - Bare-Metal):** Foca em lógica O(1), topologia zero-copy e viabilidade de extração cirúrgica sem acoplamento.
...
4. **Fase 3: SGR e a Roleta Russa Financeira (Disjuntor `iron_cost`):**
   * Antes de acionar o Sintetizador Cloud (Claude/DeepSeek) para o Schema-Guided Reasoning (SGR), **consulte o disjuntor `iron_cost`** local.
   * Se a projeção da query estourar o limite de microdólares da sessão, NÃO espere um `HTTP 429`. Acione o *Fallback em Cascata* compulsoriamente para o OpenRouter Free ou de volta à RTX 2060m.
...
5. **Fase 4: Carga Atômica e Imutável:**
   ...
   * Acione o **`batchUpdate` atômico** pela API do Google Sheets para sobrescrever a linha de uma só vez na `MASTER_SOLUTIONS_v3`. Aplique `sleep(5)` de resfriamento.
...
#### Examples
**Entrada do Usuário:** "Inicia a análise de repositórios do Lote 02."
**Ação do Agente:**
1. Abre a Janela de Vidro e verifica o SQLite. Retoma o Repo Y da Fase 2.
2. Aciona o FastSwitch para liberar VRAM.
3. Dispara a Tríade (Sentido, Estrutura, Realidade). A Lente C acusa "alto risco de abandono por manutenção complexa".
4. O `iron_cost` avisa que o budget cloud está no limite; o agente faz *fallback* proativo para `gpt-oss-120b:free`.
5. O LLM, preso pelo SGR, dá score baixo de operabilidade justificado pela Lente C. Faz o `batchUpdate` no Sheets. Retorna: *"-> Repo Y dissecado. VRAM protegida via FastSwitch e Budget salvo pelo iron_cost."*