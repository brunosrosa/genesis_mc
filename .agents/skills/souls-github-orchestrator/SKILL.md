---
name: souls-github-orchestrator
description: O Ditador GitOps e Mestre do Fluxo SOULS. Impõe 'No Ticket, No Code' e Shadow Workspaces O(1). Delega a sincronia da Tríade de Memória para o Chyros Daemon (Consistência Eventual). Protege a concorrência via Mutex de Arquivos e roteia aprovações via Trust Score Dinâmico (EMA/ELO) prevenindo a Fadiga de Aprovação.
triggers: ["souls-github-orchestrator", "gerenciar fluxo", "gitops", "atualizar kanban", "issue", "pull request", "fechar tarefa", "submeter pr", "repo_meta"]
---

### skill: SOULS GitHub Orchestrator (O Ditador GitOps V4.0 Supremo)
...
5. **Protocolo de PR Limpo e Code Review Humano (HITL):**
   * Ao concluir as mutações com `Exit Code 0`, o agente DEVE empurrar a branch para o origin (`git push origin <branch>`) e abrir o Pull Request remotamente utilizando a CLI do GitHub (`gh pr create`) ou fornecendo a URL de criação de PR direta do GitHub.
   * Fica terminantemente proibido tentar realizar merges síncronos manuais diretos na branch `main`/infraestrutura no background, garantindo a integridade e o **Code Review Humano (HITL)**.
   * Se a CLI `gh` não estiver instalada ou autenticada, o agente DEVE gerar e devolver ao Arquiteto Humano a URL canônica de criação de PR (`https://github.com/<owner>/<repo>/compare/<base>...<head>?expand=1`) para autorização manual — NUNCA merge automático.

6. **Governança de Aprovação (Dynamic Trust Scoring):**
...
#### Constraints
* **PROIBIÇÃO DA SINCRONIA LETAL:** Sincronizar o LanceDB na mesma thread da interface gráfica corrompe a arquitetura SOULS. O *Daemon Chyros* é a sua válvula de escape.
