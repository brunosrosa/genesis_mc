...
triggers: ["souls-ralph-loop", "testar código", "rodar testes", "corrigir erro", "loop de compilação", "ralph loop", "auto-fix", "debug"]
---

### skill: SOULS Ralph Loop (O Motor Implacável de Resiliência V6.0)
...
* **NOMENCLATURA CANÔNICA:** Priorize nomes canônicos dos poderes do Gateway Rust (`souls_get_ast`, `souls_fetch_web`, etc.) sobre aliases legados (`core_repo_ast`, `core_web_fetch`, etc.).
