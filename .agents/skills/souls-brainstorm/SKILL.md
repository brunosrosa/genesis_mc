...
2. **Fase Map: O Conselho Invisível via MCP (`sequentialthinking`):**
   ...
   * Invoque imediatamente a ferramenta `sequentialthinking`. No *background*, cruze a ideia contra as quatro lentes do SOULS usando os construtos de pensamento (*Regular, Revision, Branching*):
...
4. **Fase Reduce: O Handoff para o `proposal.md`:**
   * Após a validação em *background*, exiba no chat APENAS a síntese destilada e peça aprovação do Arquiteto para gravar no arquivo `docs/proposal.md`. O artefato DEVE conter:
     ...
     * **Red Lines (Limites):** O que é TERMINANTEMENTE PROIBIDO fazer nesta funcionalidade (para guiar o `@souls-sdd`).
