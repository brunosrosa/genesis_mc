...
fn clone_or_fetch_to_workspace_blocking(
    ...
    let nonce = format!("{}_{}", std::process::id(), souls_mc_lib::telemetry::now_epoch_nanos());
...
fn required_github_token() -> Result<String, GithubTrackerError> {
    ...
    if let Some(token) = read_local_env_var("GITHUB_PAT") {
        return Ok(token);
    }
    #[cfg(test)]
    {
        Ok("mock_pat".to_string())
    }
    #[cfg(not(test))]
    Err(GithubTrackerError::MissingGithubToken)
}
