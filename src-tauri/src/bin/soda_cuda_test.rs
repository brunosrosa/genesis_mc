...
// Usage: cargo run --bin soda_cuda_test --features llama_backend -- [path_to_model.gguf]
...
mod test_impl {
    ...
    pub fn run() -> Result<(), Box<dyn std::error::Error>> {
        println!("=== SODA CUDA ATOMIC TEST (Compute Capability 7.5 - RTX 2060m) ===");
        ...
        if !model_path.exists() {
            ...
            eprintln!("Example: cargo run --bin soda_cuda_test --features llama_backend -- C:\\path\\to\\model.gguf");
            return Ok(());
        }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    {
        eprintln!("ERROR: The binary `soda_cuda_test` requires the `llama_backend` feature flag.");
        eprintln!("Please run with: cargo run --bin soda_cuda_test --features llama_backend");
