...
///
/// Expõe a função `souls_clean_path` que:
...
pub fn souls_clean_path(path: &Path) -> PathBuf {
    let clean_path = souls_strip_unc_prefix(path);
...
/// Usada internamente por `souls_clean_path` e diretamente em contextos de testes puros.
pub fn souls_strip_unc_prefix(path: &Path) -> PathBuf {
...
mod tests {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // RED TESTS — souls_strip_unc_prefix (puro, sem filesystem)
    // ═══════════════════════════════════════════════════════════════════════
    ...
    fn test_unc_extended_prefix_is_stripped() {
        ...
        let result = souls_strip_unc_prefix(input);
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // RED TESTS — souls_clean_path (com canonicalize para paths existentes)
    // ═══════════════════════════════════════════════════════════════════════

    #[test]
    fn test_souls_clean_path_nonexistent_path_strips_unc() {
        // Para paths não-existentes, apenas remove prefixo — sem canonicalize
        let input = Path::new(r"\\?\Z:\souls-ephemeral-test-nonexistent\subdir");
        let result = souls_clean_path(input);
        assert_eq!(result, PathBuf::from(r"Z:\souls-ephemeral-test-nonexistent\subdir"),
            "Path nao-existente deve ter prefixo removido sem canonicalize");
    }

    #[test]
    fn test_souls_clean_path_normal_path_is_returned_as_is() {
        let input = Path::new(r"Z:\non-existent-souls-dir");
        let result = souls_clean_path(input);
    ...
    #[test]
    fn test_souls_clean_path_existing_temp_dir_has_no_unc() {
        // O diretório temporário real do sistema NUNCA deve retornar com \\?\
        let temp_dir = std::env::temp_dir();
        let result = souls_clean_path(&temp_dir);
        let result_str = result.to_string_lossy();
        assert!(!result_str.starts_with(r"\\?\"),
            "souls_clean_path em diretório existente nao deve conter \\\\?\\: {result_str}");
    ...
    #[cfg(target_os = "windows")]
    #[test]
    fn test_souls_clean_path_on_std_temp_dir_is_rooted() {
        // O resultado deve ser um path absoluto com letra de drive
        let temp_dir = std::env::temp_dir();
        let result = souls_clean_path(&temp_dir);
        assert!(result.is_absolute(),
            "souls_clean_path deve retornar path absoluto: {}", result.display());
