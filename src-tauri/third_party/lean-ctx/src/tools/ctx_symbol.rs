    ...
    let mut results: Vec<&SymbolEntry> = index
        ...
        .filter(|s| {
            ...
            let file_match = file_filter
                .map(|f| file_matches_filter(&s.file, f))
                .unwrap_or(true);
...
fn file_matches_filter(indexed_file: &str, requested_filter: &str) -> bool {
    let indexed = normalize_path_for_match(indexed_file);
    let requested = normalize_path_for_match(requested_filter);
    ...
    match (
        Path::new(indexed_file).file_name(),
        Path::new(requested_filter).file_name(),
    ) {
        (Some(indexed_name), Some(requested_name)) => indexed_name == requested_name,
        _ => false,
    }
}
...
mod tests {
    ...
    #[test]
    fn find_with_absolute_file_filter() {
        let index = test_index();
        let results = find_symbols(&index, "main", Some("Z:\\repo\\src\\main.rs"), None);
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].name, "main");
    }
