...
mod tests {
    ...
    fn get_int_accepts_numeric_strings() {
        ...
        assert_eq!(get_int(&args, "top_k"), Some(15));
    }

    #[test]
    fn get_int_ignores_non_numeric_strings() {
        let args = Some(serde_json::Map::from_iter([(
            "top_k".to_string(),
            Value::String("REQUEST_TIMEOUT".to_string()),
        )]));
        assert_eq!(get_int(&args, "top_k"), None);
    }
}
