...
pub fn execute(language: &str, code: &str, timeout_secs: Option<u64>) -> SandboxResult {
    ...
    let runtime = match resolve_runtime(&normalized_language) {
        Ok(r) => r,
        Err(stderr) => {
...
fn resolve_runtime(language: &str) -> Result<RuntimeConfig, String> {
    let lang = normalize_language(language);
    let lang = lang.as_str();

    match lang {
        "javascript" | "js" | "node" | "nodejs" => Ok(RuntimeConfig {
            command: find_binary(&["bun", "node"]).ok_or_else(|| runtime_missing(language, &["bun", "node"]))?,
            args: vec!["-e".to_string()],
            needs_temp_file: false,
            file_extension: "js".to_string(),
            env: HashMap::new(),
        }),
        "typescript" | "ts" | "tsx" => Ok(RuntimeConfig {
            command: find_binary(&["bun", "npx"]).ok_or_else(|| runtime_missing(language, &["bun", "npx"]))?,
        ...
        "python" | "py" | "python3" => Ok(RuntimeConfig {
            command: find_binary(&python_candidates())
                .ok_or_else(|| runtime_missing(language, &python_candidates()))?,
        ...
        "shell" | "bash" | "sh" => {
            ...
            {
                Ok(RuntimeConfig {
                    command: find_binary(&["bash", "sh"]).ok_or_else(|| runtime_missing(language, &["bash", "sh"]))?,
                    args: vec!["-c".to_string()],
                    needs_temp_file: false,
        ...
        "ruby" | "rb" => Ok(RuntimeConfig {
            command: find_binary(&["ruby"]).ok_or_else(|| runtime_missing(language, &["ruby"]))?,
            args: vec!["-e".to_string()],
            needs_temp_file: false,
            file_extension: "rb".to_string(),
            env: HashMap::new(),
        }),
        "go" | "golang" => Ok(RuntimeConfig {
            command: find_binary(&["go"]).ok_or_else(|| runtime_missing(language, &["go"]))?,
            args: vec!["run".to_string()],
            needs_temp_file: true,
            file_extension: "go".to_string(),
            env: HashMap::new(),
        }),
        "rust" | "rs" => Ok(RuntimeConfig {
        ...
        "php" => Ok(RuntimeConfig {
            command: find_binary(&["php"]).ok_or_else(|| runtime_missing(language, &["php"]))?,
            args: vec!["-r".to_string()],
            needs_temp_file: false,
            file_extension: "php".to_string(),
            env: HashMap::new(),
        }),
        "perl" | "pl" => Ok(RuntimeConfig {
            command: find_binary(&["perl"]).ok_or_else(|| runtime_missing(language, &["perl"]))?,
            args: vec!["-e".to_string()],
            needs_temp_file: false,
        ...
        "r" => Ok(RuntimeConfig {
            command: find_binary(&["Rscript"]).ok_or_else(|| runtime_missing(language, &["Rscript"]))?,
            args: vec!["-e".to_string()],
            needs_temp_file: false,
            file_extension: "R".to_string(),
            env: HashMap::new(),
        }),
        "elixir" | "ex" => Ok(RuntimeConfig {
            command: find_binary(&["elixir"]).ok_or_else(|| runtime_missing(language, &["elixir"]))?,
            args: vec!["-e".to_string()],
            needs_temp_file: false,
...
fn runtime_missing(language: &str, candidates: &[&str]) -> String {
    format!(
        "Runtime for '{language}' is not available in PATH. Looked for: {}",
        candidates.join(", ")
    )
}
...
mod tests {
    ...
    #[test]
    fn normalize_language_trims_and_lowercases() {
        assert_eq!(normalize_language(" Python3 "), "python3");
        assert_eq!(normalize_language("NODEJS"), "nodejs");
    }

    #[test]
    fn python_candidates_include_py_on_windows() {
        let candidates = python_candidates();
        #[cfg(target_os = "windows")]
        assert_eq!(candidates[0], "py");
        #[cfg(not(target_os = "windows"))]
