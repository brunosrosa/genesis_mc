...
/// Whether to render special/control tokens as text when detokenising.
///
/// Mirrors `llama-cpp-2`'s `model::Special`. The ik wrapper upstream exposes
/// only a `bool special` knob; SOULS MC adds this enum for parity with the
/// llama-cpp-2 API used by `llama_engine.rs` (Marco IV).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Special {
    /// Render special tokens as raw text (e.g. `<|im_start|>` visible).
    Tokenize,
    /// Treat special tokens as control; do not emit text for them.
    Control,
}

impl Special {
    /// Map onto the underlying `bool` (matches ik's `llama_token_to_piece` flag).
    #[must_use]
    pub const fn as_bool(self) -> bool {
        matches!(self, Self::Tokenize)
    }
}
...
impl LlamaModel {
    ...
    // ========================================================================
    // SOULS MC Marco IV shim — parity with `llama-cpp-2`'s `token_to_str`.
    // The ik wrapper upstream exposes `token_to_piece` (bool special) but no
    // shortcut for the common `String` case. This is the convenience shim that
    // `llama_engine.rs:451` uses (`model.token_to_str(token, llama_cpp_2::model::Special::Tokenize)`).
    // ========================================================================

    /// Convert a single token to its text piece (UTF-8 lossy).
    ///
    /// # Errors
    ///
    /// [`LlamaError::TokenToPiece`] if the C conversion fails.
    pub fn token_to_str(&self, token: LlamaToken, special: Special) -> Result<String, LlamaError> {
        let bytes = self.token_to_piece_bytes(token, 8, special.as_bool(), None)?;
        Ok(String::from_utf8_lossy(&bytes).into_owned())
    }
}
