...
Fica decretada a separação física e matemática entre a "Ação" (GPU) e a "Avaliação Rápida" (CPU). O SOULS institui a figura do "Hipocampo Epistêmico", operando sob as seguintes leis imutáveis:
...
*   Medindo o delta da probabilidade estrita de *tokens* âncora (ex: calculando a diferença angular/matemática de ativação entre os *logits* mapeados para risco vs. segurança), o SOULS extrai a certeza da IA de forma determinística, sem nunca decodificar uma string.
...
*   Para prover acesso direto aos ponteiros da memória de *logits*, o SOULS fará o uso cirurgicamente algemado de bibliotecas focadas em *prefill* (ex: via injeção C-FFI de `llama-cpp-4` contida ou suporte especializado), assegurando extrações atômicas [2, 6, 7].
