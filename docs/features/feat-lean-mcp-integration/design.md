...
núcleo do SODA. A pasta `third_party/lean-ctx/` permanece apenas como cadáver READ-ONLY.
**Princípio:** "SODA dita as regras. O cadáver é apenas estudo. Nenhum `rmcp` no nosso binário."
...
flowchart TB
    ...
    subgraph SODA[Gateway Rust - souls_mc - SEM dependência lean-ctx]
        direction TB
        ...
        subgraph M2[Router MCP - soda_mcp_server.rs]
            R1[handle_mcp]
...
| **W3** | Transplante das 2 vitais (T2) | `src/bin/soda_mcp_server.rs` — `souls_read` + `souls_delta_diff` | `cargo check --bin soda_mcp_server` Exit 0 |
| **W4** | Stubs renomeados (T3) | `src/bin/soda_mcp_server.rs` — 15 stubs | `grep "STUB:" /dev/null` retorna vazio |
| **W5** | Validação (Fase 5) | `cargo check --bin soda_mcp_server` + diff atômico | Exit 0 + Blast Radius ≤ 5 arquivos novos |

**Isolamento:** 5 arquivos novos em `cognition/lean_vacuum/`, 1 arquivo modificado
(`soda_mcp_server.rs`), 1 arquivo modificado (`core/mod.rs`). Zero colisões.
...
síncronas e podem ser invocadas tanto do `soda_mcp_server` (NDJSON) quanto de um futuro
...
8. **SEMPRE** `cargo check --bin soda_mcp_server` Exit 0 antes do HITL.
...
- [ ] `grep "STUB:" src/bin/soda_mcp_server.rs` retorna vazio.
