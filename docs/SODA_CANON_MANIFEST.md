# SODA CANON MANIFEST (V6 - SOULS MC ERA)
...
Este documento é a Constituição do Sovereign Operating Data Architecture (SODA). Ele ancora a cognição dos Agentes e dita as leis absolutas para a avaliação, canibalização e desenvolvimento de qualquer software no nosso ecossistema.

## I. IDENTIDADE E PROPÓSITO: O "SOUL"
O SODA (sob a identidade de produto Souls MC) não é um "assistente de chat" ou um wrapper de IA em nuvem. Ele é um **Exoesqueleto Cognitivo** e uma **Prótese de Função Executiva** desenhada para mitigação de "Flow-Debt" (Dívida de Fluxo), focado primariamente em usuários neurodivergentes (TDAH/2e). 
...
Toda arquitetura candidata a entrar no ecossistema SODA deve sobreviver ao nosso "Treino de Gravidade": operação fluida em um Intel i9, 32GB de RAM e estritos **6GB de VRAM (NVIDIA RTX 2060m)**.
...
* **Agnosticismo Matemático:** A "alma matemática" do SODA é transmutável. Os algoritmos devem ser projetados para rodar tanto na CPU (via instruções AVX2) quanto em GPUs via abstrações nativas (lamma-cpp-2 ou 4, Burn/CubeCL, Candle), permitindo elasticidade.

## III. CYBER-NEURO SYNTHESIS E UX ESTRITA
A interface do SODA é o silêncio visual absoluto. Construída estritamente em **Svelte 5 + Tauri v2**. A UI é passiva e não possui regras de negócio.
* **A Morte do Virtual DOM:** Frameworks como React estão banidos. O sistema usa Svelte Runes para reatividade cirúrgica.
* **Tiling Window Manager 2D e Zero Layout Shift:** O SODA repudia janelas flutuantes caóticas e "Liquid Glass" (blur) que destroem a renderização da GPU. A interface adota planaridade ortogonal (Mosaico 2D). Modais expansivos que causam refusão de layout são proibidos. O uso da GenUI ("Flips Efêmeros") garante injeção de interface sem quebrar o espaço cognitivo.
...
O SODA repudia o RAG (Retrieval-Augmented Generation) tradicional de busca cega. Ele atua sob o paradigma do **LLM Wiki**.
...
A conclusão de qualquer análise deve resultar na Síntese Executiva: uma decisão fria ditando o que o SODA vai amputar e assimilar (Ouro a Extrair), repudiando o lixo da stack original.