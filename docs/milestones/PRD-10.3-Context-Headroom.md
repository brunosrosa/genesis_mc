...
**Módulo:** Gateway Tokio HTTP/Proxy (`soda-router`, `soda-ccr` / `headroom_engine.rs`)  
...
O objetivo é implementar no Gateway Tokio Rust do SODA:
...
2. **Poda Semântica Zero-Copy (`soda-router`):** Compressão de ASTs de código via `tree-sitter`, resumos JSON via `simd-json` e heurísticas de logs.
3. **Compressão Reversível CCR Zero-VRAM (`soda-ccr`):** Armazenamento do histórico original bruto 100% na RAM do Host (Intel i9) via `DashMap` concorrente e interceção *loopback* local da tool `headroom_retrieve(hash)` em latência $< 1.0 \text{ ms}$.
...
* **Módulo:** `soda-ccr` / `headroom_engine::tests`
...
* **Módulo:** `soda-router` / `code_compressor::tests`
...
* **Módulo:** `soda-router` / `budget_evaluator::tests`
